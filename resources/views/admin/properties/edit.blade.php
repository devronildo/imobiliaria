         ...
         @if($errors->all())
           ...
           @if(session()->exists('message'))
             @message(['color' => session()->get('color')])
              <p class="icon icon-asterisk">{{ session()->get('message') }}</p>
             @endmessage
           @endif
            <ul class="nav_tabs">
                        ...
                        <div class="label_g2">
                            <label class="label">
                                ...
                                <select name="category" class="select2">
                                    ...
                                    <option value="terrain" {{ (old('category') == 'Terreno'  ? 'selected' : ($property->category == 'Terreno' ? 'selected' : '' )) }}>Terreno</option>
                                </select>
                            </label>
                                ...
                                <div class="property_image_item">
                                   <img src="{{ $image->url_cropped }}" alt="">
                                    <div class="property_image_actions">
                                       ...
                                       class="btn btn-small {{ ($image->cover == true ? 'btn-green' : '') }} icon-check icon-notext img-set-cover" data-action="{{ route('admin.properties.imageSetCover', ['image' => $image->id]) }}"></a>
                                       <a href="javascript:void(0)"
                                       class="btn btn-red btn-small icon-times icon-notext image-remove" data-action="{{ route('admin.properties.imageRemove', ['image' => $image->id]) }}"></a>
                                    </div>
                                </div>
                              @endforeach
                           </div>
...
<script>
    $(function () {
        ...
        $('.img-set-cover').click(function(event){
